...
awk '/^>/ {p=($0 ~ /lncRNA/)} p' /lustre/smriti.a/samhita/reference/gencode.v47.transcripts.fa | awk '/^>/ {header=$0} /lncRNA/ {print header; print}' | awk 'BEGIN {RS=">"; ORS=""} NR>1 {print ">"$0}' | awk -F'|' 'BEGIN {OFS="\t"; print "Transcript_ID", "Gene_ID", "Transcript_Name", "Gene_Name", "Length", "Biotype"} \
           {split($1, a, "|"); print substr($1,2), $2, $5, $6, $7, $8}' > /lustre/smriti.a/samhita/reference/lncRNA_ids.txt

awk '/^>/ {p=($0 ~ /protein_coding[^_]/)} p' /lustre/smriti.a/samhita/reference/gencode.v47.transcripts.fa | awk '/^>/ {header=$0} /protein_coding[^_]/ {print header; print}' | awk 'BEGIN {RS=">"; ORS=""} NR>1 {print ">"$0}' | awk -F'|' 'BEGIN {OFS="\t"; print "Transcript_ID", "Gene_ID", "Transcript_Name", "Gene_Name", "Length", "Biotype"} \
           {split($1, a, "|"); print substr($1,2), $2, $5, $6, $7, $8}' > /lustre/smriti.a/samhita/reference/protein_coding_ids.txt
...
samtools view /lustre/smriti.a/samhita/results_dengue_80/IGIB1130430586n_Nextseq_Negative/star_gencode/IGIB1130430586n_Nextseq_Negative_Aligned.sortedByCoord.out.bam |
  ...
  rsync -avP /lustre/smriti.a/samhita/Radhika_diabetes_mice_cellLines/raw_fastqs samhita.p@tejas2.igib.res.in:/lustre/samhita.p/Radhika_diabetes_mice_cellLines
