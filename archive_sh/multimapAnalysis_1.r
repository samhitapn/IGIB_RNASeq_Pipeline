...
####### STATS FROM GTF
gtfData <- import("/lustre/smriti.a/samhita/reference/gencode.v47.annotation.gtf")
gtf <- gtfData[mcols(gtfData)$type == "gene"]
...
rmskData_gr <- makeGRangesFromDataFrame(rmskData,keep.extra.columns=TRUE)                   
# rmskData_gr <- GRanges(seqnames = rmskData$seqname,
#                    ranges = IRanges(start = rmskData$start, end = rmskData$end),
#                    strand = rmskData$strand,
#                    repeat_name = rmskData$repeat,
#                    class_family = rmskData$class_family,
#                    perc_div = rmskData$perc_div)
